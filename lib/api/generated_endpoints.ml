...
let create_user (req : Dream.request) =
  ...
  let body =
    Generated_types.CreateUserInput.t_of_yojson (Yojson.Safe.from_string body)
  in
  let* (result :
         (Generated_types.CreateUserOutput.t, Dream.response Lwt.t) result) =
    Handlers.Server.create_user req body
  in
  match result with
  | Ok result ->
      result |> Generated_types.CreateUserOutput.yojson_of_t
...
let users (req : Dream.request) =
  ...
  | Ok query -> (
      let* (result :
             (Generated_types.UsersOutput.t, Dream.response Lwt.t) result) =
        Handlers.Server.users req query
      in
      match result with
      | Ok result ->
          result |> Generated_types.UsersOutput.yojson_of_t
          |> Yojson.Safe.to_string |> Dream.json
      | Error response -> response)

let get_user (req : Dream.request) =
  let user_id = Dream.param req "user_id" in
  let* (result : (Generated_types.GetUserOutput.t, Dream.response Lwt.t) result)
      =
    Handlers.Server.get_user req user_id
  in
  match result with
  | Ok result ->
      result |> Generated_types.GetUserOutput.yojson_of_t
      |> Yojson.Safe.to_string |> Dream.json
  | Error response -> response
...
let delete_user (req : Dream.request) =
  let user_id = Dream.param req "user_id" in
  let* (result :
         (Generated_types.DeleteUserOutput.t, Dream.response Lwt.t) result) =
    Handlers.Server.delete_user req user_id
  in
  match result with
  | Ok result ->
      result |> Generated_types.DeleteUserOutput.yojson_of_t
      |> Yojson.Safe.to_string |> Dream.json
  | Error response -> response
...
let add_users_to_conversation (req : Dream.request) =
  ...
  let* body = Dream.body req in
  let body =
    Generated_types.AddUsersToConversationInput.t_of_yojson
      (Yojson.Safe.from_string body)
  in
  let* (result :
         ( Generated_types.AddUsersToConversationOutput.t,
           Dream.response Lwt.t )
         result) =
  ...
  match result with
  | Ok result ->
      result |> Generated_types.AddUsersToConversationOutput.yojson_of_t
      |> Yojson.Safe.to_string |> Dream.json
  | Error response -> response
...
let remove_users_from_conversation (req : Dream.request) =
  ...
  let* body = Dream.body req in
  let body =
    Generated_types.RemoveUsersFromConversationInput.t_of_yojson
      (Yojson.Safe.from_string body)
  in
  let* (result :
         ( Generated_types.RemoveUsersFromConversationOutput.t,
           Dream.response Lwt.t )
         result) =
  ...
  match result with
  | Ok result ->
      result |> Generated_types.RemoveUsersFromConversationOutput.yojson_of_t
      |> Yojson.Safe.to_string |> Dream.json
  | Error response -> response
